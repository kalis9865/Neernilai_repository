...
function getAllDevices() {
  ...
  $.ajax({
    ...
    success: function(response) {
      console.log("response for select device"+response)
      ...
      deviceOptions += "<option data-mac='"+ value["mac_id"] +"' value='" + value["device_id"] + "'>" + value["device_name"] + "</option>";
        ...
        $('#selectDevice').html(deviceOptions);
        // Not going to load first device sensors by default //
        /*$.each(result.devices[0]["sensors"], function(key, value) {
          firstDeviceSensors += "<option value='" + value["id"] + "'>" + value["name"] + "</option>";
        });
        $(e.currentTarget).find('#selectSensor').html(firstDeviceSensors);*/
      }
    }
  });
}
...
$("#selectDevice").on('change', function(e) {
  ...
  $.ajax({
    ...
    success: function(response) {
      console.log("response for select sensor"+response)
      var result = JSON.parse(response);
      firstDeviceSensors = "";
      ...
      $('#selectSensor').html(firstDeviceSensors);
      //$('#selectSensor').change(); Not going to trigger get widgets by default
    }
  });
});

// SelectSensor Event Listener //
/*$("#selectSensor").on('change', function(e) {
  var tempWidgetTypes = widgetTypes;
  var options = "";
  switch ($(this).val()) {
    case "ambient":
      var options = "";
      $.each(tempWidgetTypes, function(key, val) {
        if (key != 'temp' && key != 'tank' && key != 'pressure') {
...
$("#addStack").on("click", function() {
  ...
  let selectedDeviceVal = $("#selectDevice option:selected").val();
  let selectSensorVal = $("#selectSensor option:selected").val();
  let chartTypeVal = $("#chartType option:selected").val();
  if(selectedDeviceVal=="select" || selectSensorVal=="select" || chartType=="select"){
      return;
  }

  var si_unit;
  var widgetIdPrefix = $("#selectDevice option:selected").data('mac');
  var title = $("#selectDevice option:selected").text() + ":";
  widgetIdPrefix += "_"+$("#selectSensor option:selected").text();
  switch ($("#selectSensor").val()) {
    case "1":
    // case "ambient":
  ...
  var serializedData;
  widgetIdPrefix += "_"+$("#chartType option:selected").text();
  switch ($("#chartType :selected").val()) {
    case "2":
      //widgetIdPrefix += "_line";
      serializedData = [{
        "w": 6,
        "h": 5,
        "minW": 3,
...
$("#compactLayout").on('click', function(e) {
  grid.compact();
});
...
// Save Dashboard //
$("#createNewDashboard").on('click', function(e) {
  if ($("#inputDashboardName").val() == "") {
    flash.error("Please enter waterbody name");
  } else {

	//serializedData = grid.save();
    //serializedFull = grid.save();
